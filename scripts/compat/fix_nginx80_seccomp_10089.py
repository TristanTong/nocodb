#!/usr/bin/env python3
"""Fix nginx pid/seccomp on 100.89 and verify :80 proxy."""
import os
import sys
import time
import paramiko
...
def main():
    ...
    try:
        ...
        with sftp.file(f"{REMOTE}/nginx.conf", "w") as f:
            f.write(NGINX.replace("\r\n", "\n"))
        with sftp.file(f"{REMOTE}/docker-compose.yml", "w") as f:
            f.write(COMPOSE.replace("\r\n", "\n"))
        sftp.close()

        run(c, "docker rm -f mlnocodb-nginx 2>/dev/null || true")
        run(c, f"cd {REMOTE} && docker compose up -d nginx 2>&1")
        time.sleep(5)
        run(c, "docker ps -a --filter name=mlnocodb --format 'table {{.Names}}\\t{{.Status}}\\t{{.Ports}}'")
        run(c, "docker logs --tail 15 mlnocodb-nginx 2>&1")

        run(c, "curl -s -m 10 -o /dev/null -w 'UI80=%{http_code}\\n' http://127.0.0.1/")
        run(
            c,
        ...
        run(
            c,
            "curl -s -m 8 -o /tmp/j.txt -w 'jobs=%{http_code} ctype=%{content_type} t=%{time_total}\\n' "
            "-X POST http://127.0.0.1/jobs/listen -H 'Content-Type: application/json' "
            "-d '{\"_mid\":0,\"data\":{\"id\":\"x\"}}'; head -c 80 /tmp/j.txt; echo",
            timeout=25,
        )
        run(c, "curl -s -m 5 -o /tmp/dl.txt -w 'dl=%{http_code}\\n' http://127.0.0.1/dl/a/b/c; head -c 40 /tmp/dl.txt; echo")
    finally:
        c.close()
    return 0
