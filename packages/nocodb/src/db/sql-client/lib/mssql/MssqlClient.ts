...
import queries from './mssql.queries';
...
class MssqlClient extends KnexClient {
  ...
  async schemaCreateWithCredentials(_args): Promise<any> {
    ...
    result.code = -1;
    result.message = 'Not implemented for MSSQL';
    return result;
  }

  get schema() {
    const searchPath = this.connectionConfig?.searchPath;
    if (Array.isArray(searchPath)) {
      return searchPath[0] || 'dbo';
    }
  ...
  async createDatabaseIfNotExists(args: { database: string }) {
    ...
    try {
      ...
      if (connectionParamsWithoutDb.connection?.port != null) {
        connectionParamsWithoutDb.connection.port = Number(
          connectionParamsWithoutDb.connection.port,
        );
        connectionParamsWithoutDb.connection.options = {
          ...(connectionParamsWithoutDb.connection.options || {}),
          port: Number(connectionParamsWithoutDb.connection.port),
        };
      }
